...
export default function ManajemenAkun() {
  ...
  const onSimpan = async () => {
    ...
    try {
      ...
      if (mode === "add") {
        ...
        setSuccessMessage("Akun posbankum berhasil dibuat!");
      } else {
  ...
  const confirmHapus = async () => {
    ...
    try {
      ...
      setSuccessMessage("Posbankum berhasil dihapus!");
      setReloadKey((x) => x + 1);
  ...
  return (
    <section className="kpShell">
      ...
      <div className="kpHeaderRow">
        <div className="kpHeading">Manajemen Akun</div>
      ...
      <DeleteConfirmModal
        open={!!deleteTarget}
        title="Hapus Posbankum?"
        subtitle="Tindakan ini tidak dapat dibatalkan"
        description={`Apakah Anda yakin ingin menghapus posbankum ${deleteTarget?.nama ? `"${deleteTarget.nama}"` : "ini"}? Semua data terkait akan dihapus permanen.`}
